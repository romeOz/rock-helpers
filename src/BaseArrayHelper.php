...
class BaseArrayHelper
{
    ...
    public static function updateValue(array $array, array $keys, callable $callback /* , $args... */)
    {
        ...
        foreach ($keys as $key) {
            if (!is_array($current) || !Helper::getValue($current[$key])) {
                throw new BaseException(sprintf('Did not find path %s in structure %s', json_encode($keys),json_encode($array)));
            }
            $current = &$current[$key];
        }
     ...
     * @throws BaseException if the $descending or $sortFlag parameters do not have
    ...
    public static function multisort(&$array, $key, $direction = SORT_ASC, $sortFlag = SORT_REGULAR)
    {
        ...
        if (is_scalar($direction)) {
            ...
            throw new BaseException('The length of $descending parameter must be the same as that of $keys.');
        }
        if (is_scalar($sortFlag)) {
            $sortFlag = array_fill(0, $n, $sortFlag);
        } elseif (count($sortFlag) !== $n) {
            throw new BaseException('The length of $sortFlag parameter must be the same as that of $keys.');
        }
        $args = [];
