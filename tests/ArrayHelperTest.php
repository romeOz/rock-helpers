...
use rockunit\common\DatabaseTestCase;
...
class ArrayHelperTest extends DatabaseTestCase
{
    ...
    /**
     * @group components
     * @dataProvider providerToArrayWithModel
     */
    public function testToArrayWithModel($expected, $actual, $only = [], $exclude = [])
    {
        if (!class_exists('\rock\components\Model')) {
            $this->markTestSkipped('Rock components not installed.');
        }
        $this->assertSame(ArrayHelper::toArray($expected, $only, $exclude, true), $actual);
    }

    public function providerToArrayWithModel()
    {
        if (!class_exists('\rock\components\Model')) {
            $this->markTestSkipped('Rock components not installed.');
        }
